...
- **Integración con AWS HTTP API Gateway (JWT Authorizer Nativo)**: Conexión directa a la nueva API HTTP Gateway (`https://op6erfwwmh.execute-api.us-east-1.amazonaws.com/v1/api/status`) con validación de tokens JWT en la capa perimetral (Edge).
- **Flujo de Inicio de Sesión Modal (Popup)**: Componente modular `<LoginButton />` que invoca `instance.loginPopup()` con el scope explícito `5f5ad1dc-7259-4d00-a29d-75f1c2b3b2f4/.default`.
...
- **Backend & Cloud:** AWS HTTP API Gateway / JWT Authorizer Nativo (`https://op6erfwwmh.execute-api.us-east-1.amazonaws.com/v1/api/status`)
...
- **v1.2.2 (Patch Fix)**: Migración de endpoint de consulta a la nueva API HTTP Gateway en AWS (`https://op6erfwwmh.execute-api.us-east-1.amazonaws.com/v1/api/status`) con validación nativa de JWT Authorizer en el borde perimetral.
- **v1.2.1**: Corrección de scope MSAL a `5f5ad1dc-7259-4d00-a29d-75f1c2b3b2f4/.default` solucionando el error HTTP 401 (Unauthorized) al solicitar el token correcto para el backend en AWS API Gateway.
